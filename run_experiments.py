...
Reads experiment definitions from experiments_config.json and runs them sequentially.
"""
...
class ExperimentRunner:
    def __init__(self, base_dir: str = None):
        ...
        # self.run_script = self.base_dir / "run_all_dockers-bm.sh"
    ...
    def save_results(self, experiment_id: str, experiment_group: str = None):
        ...
        # Handle modality if it's a list
        if isinstance(modality, list) and "shimmer" in modality:
            modality = "shimmer"
        
        # Construct the output folders dynamically (same logic as in conf.py)
        ...
        # Predictions - copy from component_output_folder
    ...
    def run_experiment(self, experiment_id: str, config_updates: Dict[str, Any] = None, description: str = "", experiment_group: str = None):
        ...
        # We load the full config to log it
        ...
        if config_updates:
            # Simple merge for logging (deep merge logic is in update_config but we can just reload the file)
            with open(self.config_file, 'r') as f:
        ...
        # Add experiment group as tag for easy filtering in ClearML
        if experiment_group:
            task.add_tags([experiment_group])
            
        # Clean shimmer directory before run
        ...
        try:
            ...
            # We don't need to capture output here as it goes to stdout/stderr which ClearML captures
            ...
            print(f"\n⚠ Experiment {experiment_id} interrupted by user")
            task.mark_stopped()
            raise
        ...
        if success:
            ...
            # Load and log metrics to ClearML
            print("Logging metrics to ClearML...")
            ...
            print("Uploading artifacts to ClearML...")
    ...
    def run_experiments(self, experiments: List[Dict[str, Any]], continue_on_failure: bool = False):
        ...
        Args:
            ...
            continue_on_failure: If True, continue running experiments even if one fails
        """
        ...
        try:
            for i, exp in enumerate(experiments, 1):
                ...
                success = self.run_experiment(exp_id, config_updates, description, experiment_group)
...
def main():
    ...
    successful, failed = runner.run_experiments(experiments, args.continue_on_failure)
